...
from wordtree_output import token_predition_write
...
def readfile_questions(filename):
    '''
    read file
    '''
    ...
    data = []
    for item in f.keys():
        for question_sentence in f[item]['question']:
            sentence = []
            label = []
            for token in question_sentence:
                sentence.append(token['text'])
                label.append(['O'])
            data.append((sentence,label))
        for answer_sentence in f[item]['answer']:
            sentence = []
            label = []
            for token in answer_sentence:
                sentence.append(token['text'])
                label.append(['O'])
            data.append((sentence, label))
    return data

def readfile_tablestore(filename):
    '''
    read file
    '''
    with open(filename, 'r') as file:
    ...
    data = []
    for item in f.keys():
        for table_sentence in f[item]:
            sentence = []
            label = []
            for token in table_sentence:
                sentence.append(token['text'])
                label.append(['O'])
            data.append((sentence,label))
    return data
...
class DataProcessor(object):
    ...
    def get_wordtree_question_examples(self, data_dir, set_type):
        """Gets a collection of `InputExample`s for the train set."""
        raise NotImplementedError()

    def get_wordtree_tablestore_examples(self, data_dir):
    ...
    @classmethod
    def _read_questions_tsv(cls, input_file, quotechar=None):
        """Reads a tab separated value file."""
        return readfile_questions(input_file)

    @classmethod
    def _read_tablestore_tsv(cls, input_file, quotechar=None):
        """Reads a tab separated value file."""
        return readfile_tablestore(input_file)


class NerProcessor(DataProcessor):
    """Processor for the CoNLL-2003 data set."""
    def get_wordtree_question_examples(self, data_dir, set_type):
        return self._create_wordtree_examples(self._read_questions_tsv(os.path.join(data_dir, "questions_{}_conll.json".format(set_type))), '{}_questions'.format(set_type))

    def get_wordtree_tablestore_examples(self, data_dir):
        return self._create_wordtree_examples(self._read_tablestore_tsv(os.path.join(data_dir, "table_data_conll.json")), 'tablestore')
    ...
    def _create_wordtree_examples(self,lines,set_type):
        examples = []
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for checkpoint in checkpoints:
            ...
            for set_type in ['train', 'dev', 'test']:
                ...
                pickle_dump_large_file(y_pred, os.path.join(args.data_dir, "y_pred_questions_{}.pkl".format(set_type)))
                token_predition_write(os.path.join(args.data_dir, "questions_{}_conll.json".format(set_type)), y_pred, 'questions')
            eval_examples = processor.get_wordtree_tablestore_examples(args.data_dir)
            eval_features = convert_examples_to_features(eval_examples, label_list, args.max_seq_length, tokenizer)
            logger.info("***** Running evaluation *****")
            logger.info("  Num examples = %d", len(eval_examples))
            logger.info("  Batch size = %d", args.eval_batch_size)
                    ...
                    for j, m in enumerate(label):
                        if j == 0:
                            ...
                            if flag != len(label_map) - 1:
                                temp_1.append(temp_label_ids_list)
                                temp_2.append(temp_logits_list)
            pickle_dump_large_file(y_pred, os.path.join(args.data_dir, "y_pred_tablestore.pkl"))
            token_predition_write(os.path.join(args.data_dir, 'table_data_conll.json'),
                                  y_pred, 'tablestore')
